...
namespace ProceduralStructures {
	public class BezierSpline {
		...
		Tangent GetInterpolatedTangent(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, float t) {
			Vector3 a, b;
			if (t >= 0.05f) {
				a = GetInterpolatedPoint(p0, p1, p2, p3, t - 0.05f);
			} else {
				a = GetInterpolatedPoint(p0, p1, p2, p3, t);
			}
			if (t < 1f) {
				b = GetInterpolatedPoint(p0, p1, p2, p3, t + 0.05f);
			} else {
				b = GetInterpolatedPoint(p0, p1, p2, p3, t);
			}
